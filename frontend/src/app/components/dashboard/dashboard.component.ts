...
@Component({
  ...
  template: `
    <div class="dashboard-page">
      ...
      <!-- Site Engineer Pending Verifications & Action Portal -->
      <div class="glass-card" style="background:linear-gradient(135deg, rgba(26,29,46,0.95), rgba(13,20,38,0.95));border:1px solid rgba(0,191,165,0.4);padding:24px;border-radius:16px" *ngIf="userRole === 'Site Engineer' || pendingDeliveries.length > 0 || pendingProcurements.length > 0">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;flex-wrap:wrap;gap:12px">
          <div style="display:flex;align-items:center;gap:10px">
            <span style="font-size:24px">⚡</span>
            <div>
              <h2 style="margin:0;font-size:18px;color:#00BFA5;font-weight:700">Pending Action Items & Site Verifications</h2>
              <p style="margin:2px 0 0 0;font-size:13px;color:#a0a3b1">Real-time queue of material dispatches and purchase orders requiring site engineer verification</p>
            </div>
          </div>
          <span class="badge badge-warning" style="font-size:12px;padding:6px 12px;font-weight:700">
            {{ pendingDeliveries.length + pendingProcurements.length }} Items Awaiting Action
          </span>
        </div>

        <div style="display:grid;grid-template-columns:repeat(auto-fit, minmax(320px, 1fr));gap:16px">
          <!-- Card 1: Pending Material Dispatches -->
          <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,193,7,0.3);border-radius:12px;padding:16px">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px">
              <span style="color:#ffc107;font-weight:700;font-size:14px">🚚 Pending Dispatches ({{ pendingDeliveries.length }})</span>
              <a routerLink="/inventory" class="view-all" style="font-size:12px">Verify Stock →</a>
            </div>
            <div *ngIf="pendingDeliveries.length === 0" style="color:#6b6f82;font-size:12px;padding:12px 0">All site material shipments are fully verified!</div>
            <div *ngFor="let item of pendingDeliveries.slice(0, 4)" style="padding:10px 0;border-bottom:1px solid rgba(255,255,255,0.05);display:flex;justify-content:space-between;align-items:center">
              <div>
                <div style="color:#fff;font-weight:600;font-size:13px">{{ item.materialName }}</div>
                <div style="color:#a0a3b1;font-size:11px">{{ item.dispatchNo }} • {{ item.dispatchedQuantity }} {{ item.unit }} by {{ item.contractorName }}</div>
              </div>
              <a routerLink="/inventory" class="btn btn-primary" style="padding:4px 10px;font-size:11px;background:linear-gradient(135deg, #00BFA5, #6C63FF)">Verify</a>
            </div>
          </div>

          <!-- Card 2: Pending Purchase Orders -->
          <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(108,99,255,0.3);border-radius:12px;padding:16px">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px">
              <span style="color:#9c95ff;font-weight:700;font-size:14px">🛒 Pending Procurement POs ({{ pendingProcurements.length }})</span>
              <a routerLink="/procurement" class="view-all" style="font-size:12px">Review POs →</a>
            </div>
            <div *ngIf="pendingProcurements.length === 0" style="color:#6b6f82;font-size:12px;padding:12px 0">No pending purchase orders requiring review.</div>
            <div *ngFor="let po of pendingProcurements.slice(0, 4)" style="padding:10px 0;border-bottom:1px solid rgba(255,255,255,0.05);display:flex;justify-content:space-between;align-items:center">
              <div>
                <div style="color:#fff;font-weight:600;font-size:13px">{{ po.vendorName }}</div>
                <div style="color:#a0a3b1;font-size:11px">{{ po.invoiceNo || 'PO' }} • ₹{{ formatCurrency(po.totalAmount) }}</div>
              </div>
              <a routerLink="/procurement" class="action-btn edit-btn" style="padding:4px 10px;font-size:11px;text-decoration:none">Review</a>
            </div>
          </div>
        </div>
      </div>
...
export class DashboardComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.loadPendingActionItems();
  }

  loadPendingActionItems() {
    this.deliveryService.getDeliveries().subscribe({
      next: (res) => {
        const allDeliveries = res.success ? res.data : (Array.isArray(res) ? res : []);
        this.pendingDeliveries = allDeliveries.filter((d: any) => d.status === 'Pending Verification');
      },
      error: () => {}
    });
    ...
    this.procurementService.getProcurements().subscribe({
      next: (res) => {
        const allProcurements = res.success ? res.data : (Array.isArray(res) ? res : []);
        this.pendingProcurements = allProcurements.filter((p: any) => p.status === 'Pending' || p.status === 'Pending Approval');
      },
      error: () => {}
    });
  }
